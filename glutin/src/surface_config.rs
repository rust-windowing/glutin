...
#[allow(missing_docs)]
#[derive(Debug, Clone)]
pub struct SurfaceConfigAttribs {
...
#[allow(missing_docs)]
#[derive(Debug)]
pub struct SurfaceConfigWrapper<T> {
    pub attribs: SurfaceConfigAttribs,
    pub(crate) config: T,
}

pub type SurfaceConfig = SurfaceConfigWrapper<platform_impl::SurfaceConfig>;

impl<T> SurfaceConfigWrapper<T> {
    /// Turns the `config` parameter into another type by calling a closure.
    #[inline]
    pub(crate) fn map_config<F, T2>(self, f: F) -> ContextBuilderWrapper<T2>
    where
        F: FnOnce(T) -> T2,
    {
        SurfaceConfigWrapper {
    ...
    #[inline]
    pub(crate) fn with_config<T2>(&self, config: T2) -> ContextBuilderWrapper<T2> {
        SurfaceConfigWrapper {
            config,
...
impl SurfaceConfigBuilder {
    ...
    pub fn build(
        ...
        platform_impl::SurfaceConfig::new(el, self)
            .map(|(attribs, config)| SurfaceConfig { attribs, config })
    }
}
