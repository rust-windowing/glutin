...
use winit::dpi;
use crate::{
Context, ContextCurrentState, CreationError, NotCurrent, PossiblyCurrent, Api, PossiblyCurrentContextCurrentState, Surface, ContextError,
PixelFormat, ContextBuilderWrapper};
...
pub type OsMesaContextBuilder<'a, CS: ContextCurrentState> = ContextBuilderWrapper<&'a OsMesaContext<CS>>;
...
impl<CS: ContextCurrentState> OsMesaContext<CS> {
    ...
    pub unsafe fn make_current_osmesa_buffer(
        ...
        match self.context.make_current_osmesa_buffer(buffer.inner_mut()) {
            Ok(()) => Ok(OsMesaContext { context: self.context, phantom: PhantomData }),
            Err(err) => {
                Err((self, err))
            }
        }
    ...
    pub unsafe fn treat_as_current<CS2: PossiblyCurrentContextCurrentState>(self) -> OsMesaContext<CS2> {
        OsMesaContext {
            context: self.context,
            phantom: PhantomData,
        }
    ...
    pub(crate) fn inner(&self) -> &osmesa::OsMesaContext { &self.context }
    pub(crate) fn inner_mut(&mut self) -> &mut osmesa::OsMesaContext { &mut self.context }
}
